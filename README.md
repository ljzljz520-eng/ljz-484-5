...
- 导出全文：每篇攻略可导出为 Markdown 文件（含全部章节与图片说明；默认仅已发布章节，`include=drafts` 时草稿章节也会导出并在标题后标注「（草稿·未发布）」，便于存档）

### 读者端（index.html / reader.html）
- 首页只展示「已发布」攻略卡片（封面、目的地、标签、连载章数、更新时间），章数由列表接口直接返回
- 阅读页左侧为章节目录，右侧为正文；草稿章节（含标题、图注）对读者完全不可见
- 支持上一章 / 下一章翻页、URL 锚点定位章节
- 正文支持常用 Markdown：标题、粗体 / 斜体、行内代码、列表、引用（支持连续多行合并）、分隔线、图片、链接
- 渲染器先整体转义再做语法替换，并过滤 `javascript:` / `data:` / `vbscript:` 等危险协议，防 XSS
